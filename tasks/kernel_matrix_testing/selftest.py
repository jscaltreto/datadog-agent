...
def selftest_prepare(ctx: Context, _: bool, component: Component, cross_compile: bool) -> SelftestResult:
    ...
    res = ctx.run(f"deva inv -e kmt.prepare --stack={stack} --component={component} --compile-only {arch_arg}", warn=True)
    if res is None or not res.ok:
...
def selftest_multiarch_test(ctx: Context, allow_infra_changes: bool) -> SelftestResult:
    ...
    res = ctx.run(f"deva inv -e kmt.test --stack={stack} --packages=pkg/ebpf --run='.*TestPatchPrintkNewline.*'", warn=True)
    if res is None or not res.ok:
